...
PNPM/Turborepo monorepo. Apps: `apps/web` is the Astro frontend, `apps/secondary-web` a second Astro app, `apps/web-host` the combined runtime host, and `apps/api` the NestJS + tRPC API. Shared code — i18n helpers, storage helpers, database-connection mechanism, an OpenAI client wrapper, reusable UI primitives, and reusable `tsconfig` presets — lives in `@jainparichay/*`, published from a separate repo (`github.com/JainParichay/packages`). Changes to shared packages belong in that repo, not here; this repo consumes them as versioned dependencies. An app may still add its own `packages/<name>` workspace under its own scope to wrap or extend a `@jainparichay/*` package. Root tests are in `tests/`, setup scripts in `scripts/`, and guides in `docs/guides/`.

## State The Boundary

Shared packages contain **mechanism only** — no database schemas, no domain types, no visual design. Anything specific to one app's data model or brand belongs in that app, not in a `@jainparichay/*` package. Concretely: `@jainparichay/db` exports `createDatabaseClient` and related connection helpers, never a schema — every app owns its own `prisma/schema.prisma` and migrations (see `apps/api/prisma/`). `@jainparichay/ui` exports layout, behavior, and structural CVA variants, never a color palette or theme — an app supplies its own brand colors via `className` on top. `@jainparichay/types`, a package that held one app's domain model, was retired for violating this rule; do not recreate an equivalent. When in doubt about whether something belongs in a shared package, ask whether a second, unrelated product could use it unchanged — if the answer depends on this app's data or design, it stays here.
