...
Shared packages (`@jainparichay/ui`, `types`, `i18n`, `storage`, `db`, `config-typescript`, `ai`) are **not** local `packages/*` workspaces here — they are published from a separate repo, [github.com/JainParichay/packages](https://github.com/JainParichay/packages), and consumed as ordinary registry dependencies. `apps/*` is normally the only source of local workspace manifests; `packages/*` only exists if an app added its own package under its own scope (for example, to wrap or extend a `@jainparichay/*` package).
...
- Put code in the narrowest durable owner. Use `@jainparichay/types` for shared contracts, `@jainparichay/ui` for reusable React UI, `@jainparichay/i18n` for locale behavior, `@jainparichay/db` for database access, and app-local modules for behavior that only one app uses. Changing any of those means making the change in [the packages repo](https://github.com/JainParichay/packages) and bumping the version here, not editing `template-jp` directly.
...
- Keep local workspace package names under the current starter scope unless the task is a scope rename. Current examples are `@workspace-starter/web`, `@workspace-starter/secondary-web`, `@workspace-starter/web-host`, and `@workspace-starter/api`. `@jainparichay/*` packages (`ui`, `types`, `i18n`, `storage`, `db`, `config-typescript`, `ai`) are external and keep their own scope regardless of this repo's scope.
- Use `workspace:*` for every internal package dependency, and the published semver range (for example `^0.1.0`) for `@jainparichay/*` dependencies. After renames, update package manifests, source imports, TypeScript `paths`, root scripts, docs, and tests together.
