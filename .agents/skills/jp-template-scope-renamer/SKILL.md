---
name: jp-template-scope-renamer
...
- Use `jp-template-workspace-changes` as well if the request also adds, removes, moves, or rewires `apps/*` or `packages/*`.
...
node .agents/skills/jp-template-scope-renamer/scripts/audit-scope-references.mjs OLD_SCOPE . --include-unscoped
```
