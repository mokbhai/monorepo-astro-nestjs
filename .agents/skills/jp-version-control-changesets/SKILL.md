---
name: jp-version-control-changesets
...
Use `jp-template-workspace-changes` as well when creating, removing, renaming, moving, or wiring `apps/*` or `packages/*`. Use `jp-trpc-feature-flow` as well when changing tRPC procedures, shared API types, auth behavior, or React tRPC consumers.
