...
- Shared libraries — `@jainparichay/ui`, `@jainparichay/types`, `@jainparichay/i18n`, `@jainparichay/storage`, `@jainparichay/db`, `@jainparichay/config-typescript`, `@jainparichay/ai` — are **not** local `packages/*` workspaces. They are published from a separate repo, [github.com/JainParichay/packages](https://github.com/JainParichay/packages), and consumed here as ordinary registry dependencies (currently pinned to `^0.1.0`). Changes to their behavior belong in that repo, not here.
...
```json
{
  "dependencies": {
    ...
    "@jainparichay/ui": "^0.1.0"
  }
}
```

`workspace:*` keeps local package resolution explicit and makes refactors easier across the monorepo. All `@jainparichay/*` packages are ESM-only except `@jainparichay/ui`; a CommonJS consumer needs `await import()` for the others. `@jainparichay/db` does not ship a generated Prisma Client — a consuming app must run `prisma generate` against it (this repo does so via `apps/api`'s `postinstall`; see `pnpm db:generate`).
